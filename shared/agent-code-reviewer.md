...
description: Use this agent when an AI coding agent has completed a code-writing task and the output needs validation before it reaches human review. This includes after test generation agents, feature implementation agents, refactoring agents, migration agents, or any agent that modifies or creates code. The agent-code-reviewer should be invoked proactively after code-writing operations complete.\n\nExamples:\n\n<example>\nContext: The user asked the api-test-writer agent to create smoke tests for the campaigns endpoint.\nuser: "Write a smoke test for the campaigns endpoint"\nassistant: "I'll use the api-test-writer agent to create the smoke test."\n<api-test-writer agent completes and creates tests/campaigns/test_campaign_smoke.py>\nassistant: "The test has been created. Now let me use the agent-code-reviewer to validate the generated code."\n<commentary>\nSince a code-writing agent (api-test-writer) just completed its task, use the agent-code-reviewer to validate the output against project conventions, check for AI-specific issues, and ensure the code integrates properly with the existing codebase.\n</commentary>\n</example>\n\n<example>\nContext: User asked for a refactoring of the APIClient class.\nuser: "Refactor the APIClient to support async operations"\nassistant: "I'll implement the async refactoring for the APIClient."\n<code changes made to framework/client.py>\nassistant: "The refactoring is complete. Let me invoke the agent-code-reviewer to validate these changes."\n<commentary>\nAfter any significant code modification by an AI agent, the agent-code-reviewer should be used to catch common AI-generated issues like hallucinated methods, missing imports, or inconsistencies with existing patterns.\n</commentary>\n</example>\n\n<example>\nContext: A test generator agent created multiple test files.\nuser: "Generate regression tests for all rule types"\nassistant: "I'll generate the regression tests for KPI, Testing Tag, and Scaling Indicator rules."\n<multiple test files created in tests/rules/>\nassistant: "Tests have been generated. Now I'll use the agent-code-reviewer to review all generated files for consistency and correctness."\n<commentary>\nWhen multiple files are generated, the agent-code-reviewer is especially important to ensure consistency across all files and verify proper integration with existing test infrastructure.\n</commentary>\n</example>
model: sonnet
color: red
---
